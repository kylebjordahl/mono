...
const is = {
  ...
  key: function (a) {
    return (
      !Reflect.has(defaultInstanceSettings, a) &&
      !Reflect.has(defaultTweenSettings, a) &&
      a !== 'targets' &&
      a !== 'keyframes'
    );
  },
...
function getRelativeValue(to, from) {
  ...
  switch (operator[0][0]) {
    case '+':
      return Number(x) + Number(y) + Number(u);
    case '-':
      return Number(x) - Number(y) + Number(u);
    case '*':
      return Number(x) * Number(y) + Number(u);
...
function normalizePropertyTweens(prop, tweenSettings) {
  const settings: any = cloneObject(tweenSettings);
  // Override duration if easing is a spring
  if (/^spring/.test(settings.easing)) {
    settings.duration = spring(settings.easing, undefined);
  }
  if (is.arr(prop)) {
    const l = prop.length;
...
function anime(params) {
  ...
  const instance: any = createNewInstance(params);
  ...
  function syncInstanceChildren(time) {
    if (!instance.reversePlayback) {
      ...
      for (let i$1 = childrenLength; i$1--; ) {
...
function stagger(val, params) {
  ...
  const easing = params.easing ? parseEasings(params.easing, undefined) : null;
...
function timeline(params) {
  ...
  tl.add = function (instanceParams, timelineOffset) {
    ...
    const insParams: any = mergeObjects(
      instanceParams,
